...
def build_watchlist_rows(tickers: list[str], short_buy=3, short_sell=-3, long_buy=2, long_sell=-2,
                          require_trend_filter=True) -> list:
    return [build_ticker_summary(t, short_buy, short_sell, long_buy, long_sell, require_trend_filter)
            for t in tickers]


def print_watchlist_table(rows: list, timestamp: str = None):
...
def main():
    ...
    def do_run():
        if watchlist_mode:
            rows = build_watchlist_rows(tickers, args.short_buy_threshold, args.short_sell_threshold,
                                         args.long_buy_threshold, args.long_sell_threshold,
                                         require_trend_filter)
            print_watchlist_table(rows)
        else:
